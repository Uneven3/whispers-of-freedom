...
func _ready() -> void:
	...
	# % lookups go through _broker (scene-owned) rather than self (this node
	# is instantiated at runtime via add_child(), so it has no owner of its
	# own to resolve unique names against).
	_stamina = _broker.get_node_or_null("%StaminaComponent")
	_ledge_service = _broker.get_node_or_null("%LedgeService")
